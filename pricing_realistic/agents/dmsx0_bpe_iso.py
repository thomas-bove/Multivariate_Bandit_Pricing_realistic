"""BPE on full X° with isotropic Matérn-1/2 kernel (ablation baseline)."""
...
from ..kernels import KERNEL_L, _matern12_isotropic
...
class IsoMatern12BPE:
    """
    **Ablation baseline:** BPE = Algorithm 1 of Li & Scarlett (2022) on the **full** discrete cover
    ``X°`` = ``ALL_ACTIONS_NORM`` (**not** Smolyak).  GP = **isotropic Matérn-1/2**
    ``exp(-‖x-x'‖_2/L)``, Li batch lengths, **active set**, UCB/LCB elimination.

    Identical to ``DMSX0BPE`` in every respect except the kernel: uses the isotropic
    ``k_iso`` instead of the tensorized product kernel ``k_Π``.  This ablation isolates
    the contribution of the tensorized kernel to the regret performance.
    ...
    def __init__(
        ...
        self._kfn       = lambda X, Y: _matern12_isotropic(X, Y, kernel_L)
